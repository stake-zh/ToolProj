...
public class PackageUtil {
    ...
    private static PackageManager getPackageManager() {
        if (packageManager == null) {
            packageManager = ApplicationStatus.getApplicationContext().getPackageManager();
        }
    ...
    public static String getVersionName() {
        try {
            if (packageInfo == null) {
                packageInfo = getPackageManager().getPackageInfo(ApplicationStatus.getApplicationContext().getPackageName(), 0);
            }
    ...
    public static boolean isApkPackageOfCurrentApp(String apkPath) {
        try {
            String currentPackageName = ApplicationStatus.getApplicationContext().getPackageName();
