...
public class ApplicationStatus {
    ...
    /**
     * A map of which Activites running in forground
     */
    private static final Map<Activity, ActivityInfo> sForgroundActivityInfo =
            new ConcurrentHashMap<Activity, ActivityInfo>();
...
//                new BaseApplication.WindowFocusChangedListener() {
//                    @Override
...
//                });
    ...
    private static void onStateChange(Activity activity, int newState) {
        ...
        if (newState == ActivityState.RESUMED) {
            assert !sForgroundActivityInfo.containsKey(activity);
            sForgroundActivityInfo.put(activity, new ActivityInfo());
        }
        ...
        if (newState == ActivityState.PAUSED) {
            sForgroundActivityInfo.remove(activity);
        }
    }
    ...
    /**
     * @return A {@link List} of all forground {@link Activity}s.
     */
    public static List<WeakReference<Activity>> getForgroundActivities() {
        List<WeakReference<Activity>> activities = new ArrayList<WeakReference<Activity>>();
        for (Activity activity : sForgroundActivityInfo.keySet()) {
            activities.add(new WeakReference<Activity>(activity));
        }
        return activities;
    }
    ...
    public static void registerStateListenerForActivity(ActivityStateListener listener,
        ...
        ActivityInfo info = sActivityInfo.get(activity);
        if (info != null && info.getStatus() != ActivityState.DESTROYED) {
    ...
    public static boolean isActivityRunning(Activity activity){
        for(WeakReference<Activity> weakReference : getRunningActivities()){
    ...
    public static boolean isAcitivytForground(Activity activity) {
        for(WeakReference<Activity> weakReference : getForgroundActivities()){
            if(weakReference.get().equals(activity))
                return true;
        }
        return false;
